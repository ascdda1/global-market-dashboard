...
const categoryLabels: Record<'All' | NewsCategory, string> = { All: 'All / 全部', Markets: 'Markets / 市场', Macro: 'Macro / 宏观', 'AI & Tech': 'AI & Tech / AI与科技', Semiconductors: 'Semiconductors / 半导体', Crypto: 'Crypto / 加密资产', China: 'China / 中国' };
...
export default function MarketInformation({ compact = false }: { compact?: boolean }) {
  ...
  return <><section className="market-information terminal-panel news-panel" id="news"><div className="panel-heading"><div><span className="panel-icon" aria-hidden="true">▤</span><div className="bilingual-text"><span>{moduleLabels.marketNews.en}</span><small>{moduleLabels.marketNews.zh}</small></div></div><button className="view-more" onClick={() => setShowAllNews((value) => !value)} type="button"><span>{showAllNews ? 'Collapse' : 'View More'}</span><small>{showAllNews ? '收起' : '查看更多'}</small><b>→</b></button></div><div className="news-filters">{categories.map((item) => <button className={category === item ? 'selected' : ''} key={item} onClick={() => { setCategory(item); setShowAllNews(false); }} type="button">{categoryLabels[item]}</button>)}</div>{(newsData?.isFallback || newsError) && <div className="panel-notice"><span>!</span>{newsError ? 'News unavailable.' : 'Fallback/mock news — not a live feed.'}</div>}<div className="news-list">{!newsData && !newsError && <p className="information-empty">Loading Market News…</p>}{visibleNews.map((item) => <NewsRow item={item} key={item.id} />)}{newsData && visibleNews.length === 0 && <p className="information-empty">No news in this category.</p>}</div><div className="panel-source"><span className={newsData?.isFallback || newsError ? 'source-badge fallback' : 'source-badge'}>{newsData?.isFallback || newsError ? 'FALLBACK / MOCK' : newsData?.source ?? 'LOADING'}</span>{newsData?.lastUpdated && <small>Updated {formatTime(newsData.lastUpdated)}</small>}</div></section><section className="market-information terminal-panel people-panel" id="people"><div className="panel-heading"><div><span className="panel-icon" aria-hidden="true">♙</span><div className="bilingual-text"><span>{moduleLabels.keyPeople.en}</span><small>{moduleLabels.keyPeople.zh}</small></div></div><button className="view-more" onClick={() => setShowAllPeople((value) => !value)} type="button"><span>{showAllPeople ? 'Collapse' : 'View More'}</span><small>{showAllPeople ? '收起' : '查看更多'}</small><b>→</b></button></div>{(peopleData?.isFallback || peopleError) && <div className="panel-notice"><span>!</span>{peopleError ? 'Key People unavailable.' : 'Fallback/mock public-information feed.'}</div>}<div className="people-list">{!peopleData && !peopleError && <p className="information-empty">Loading Key People…</p>}{visiblePeople.map((item) => <PersonRow item={item} key={item.id} />)}</div><div className="panel-source"><span className={peopleData?.isFallback || peopleError ? 'source-badge fallback' : 'source-badge'}>{peopleData?.isFallback || peopleError ? 'FALLBACK / MOCK' : peopleData?.source ?? 'LOADING'}</span>{peopleData?.lastUpdated && <small>Updated {formatTime(peopleData.lastUpdated)}</small>}</div></section></>;
}
